...
osc3 = Client(osc_ip3, osc_port3)
osc3.send(msg_route + '/start', 1)
...
def moving_average_detection(ecg_signal, past_signals, past_mean_sum, n=30, threshold=-0.075):
    """
...
while True:
  try:
    ...
    if (is_heart_beat and ignore_counter == 0):
      ...
      osc2.send(msg_route, 1, pot_value, processed_pot_val)
      osc.send(msg_route_b, 1)
      osc2.send(msg_route_b, 1)
      osc3.send(msg_route_b, 1)
      ignore_counter = ignore_window_len # reinitialize counter
    else:
      osc.send(msg_route, 0, pot_value, processed_pot_val)
      osc2.send(msg_route, 0, pot_value, processed_pot_val)
      osc2.send(msg_route_b, 0)
      if ignore_counter > 0:
